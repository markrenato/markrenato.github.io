...
var app = new Framework7({
  ...
  routes: [
    ...
    {
      path: "/teste/",
      url: "pages/pages/teste.html",
    },
    {
      path: "/teste/:id",
      url: "pages/pages/teste-detalhe.html",
    },    
    {
      path: "/evento/:id",
      url: "pages/pages/evento.html",
    },
    {
      path: "/cobertura/",
      url: "pages/pages/cobertura.html",
    },        
    {
      path: "/cobertura2/",
      url: "pages/pages/cobertura2.html",
    },
    {
      path: "/cobertura3/",
      url: "pages/pages/cobertura3.html",
    },
    {
      path: "/onboarding/",
...
// Evento Detalhe
$$(document).on("page:init", '.page[data-name="evento"]', function (page) {
	var id = page.detail.route.params.id;    
	var imagem = document.getElementById("imagem");
	imagem.src = "img/carrossel/" + id + ".jpg";
	
	$$('#resumo').html('');
	
	switch (id) {
  		case "1":
    		$$('#titulo-detalhe').text("Vídeos rápidos para redes sociais");
    		$$('#resumo').append("Descubra o poder dos vídeos rápidos para redes sociais com nosso curso envolvente e prático! Em apenas algumas aulas, você aprenderá a criar conteúdo visual impactante, otimizado para cativar sua audiência nas principais plataformas sociais. Desde técnicas de edição até estratégias de narração, este curso oferece as ferramentas e insights necessários para impulsionar sua presença online e alcançar resultados impressionantes. Junte-se a nós e transforme sua abordagem nas redes sociais com vídeos que se destacam e inspiram.");
    		$$('#data-evento').text("16/10/2024");
    		$$('#hora-evento').text("08:00");
    		break;
  		case "2":
    		$$('#titulo-detalhe').text("Formação: Texto jornalístico (teoria e prática)");
    		$$('#data-evento').text("16/10/2024");
    		$$('#hora-evento').text("08:00");
    		break;
  		case "3":
    		$$('#titulo-detalhe').text("Posicionamento de imagem");
    		$$('#data-evento').text("16/10/2024");
    		$$('#hora-evento').text("08:00");
    		break;    	
    	default:
			$$('#titulo-detalhe').text("Nada");
	}
	
	$$('#resumo').append("Uma descrição sobre o evento.");
});
...
$$(document).on("page:init", '.page[data-name="cobertura"]', function (e) {
  ...
  $$("#card-cobertura").on("click", function () {
    photoBrowserDark.open();
  });
});
